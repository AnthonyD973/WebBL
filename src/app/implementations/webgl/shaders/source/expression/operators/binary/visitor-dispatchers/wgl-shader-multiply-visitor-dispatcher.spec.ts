...
describe(WglShaderMultiplyVisitorDispatcher.name, () => {
    ...
    it('should accept the operation between types for which it can be applied', () => {
        ...
        vd.visit(s.bTrue.type, s.m23.type);
        vd.visit(s.bTrue.type, s.m32.type);
        vd.visit(s.bTrue.type, s.m3.type);
        vd.visit(s.bTrue.type, s.v2.type);
        vd.visit(s.bTrue.type, s.v3.type);

        vd.visit(s.fPi.type, s.bTrue.type);
        vd.visit(s.fPi.type, s.fPi.type);
        vd.visit(s.fPi.type, s.iNeg.type);
        vd.visit(s.fPi.type, s.m23.type);
        vd.visit(s.fPi.type, s.m32.type);
        vd.visit(s.fPi.type, s.m3.type);
        vd.visit(s.fPi.type, s.v2.type);
        vd.visit(s.fPi.type, s.v3.type);

        vd.visit(s.iNeg.type, s.bTrue.type);
        vd.visit(s.iNeg.type, s.fPi.type);
        vd.visit(s.iNeg.type, s.iNeg.type);
        vd.visit(s.iNeg.type, s.m23.type);
        vd.visit(s.iNeg.type, s.m32.type);
        vd.visit(s.iNeg.type, s.m3.type);
        vd.visit(s.iNeg.type, s.v2.type);
        vd.visit(s.iNeg.type, s.v3.type);

        vd.visit(s.m32.type, s.bTrue.type);
        vd.visit(s.m32.type, s.fPi.type);
        vd.visit(s.m32.type, s.iNeg.type);
        vd.visit(s.m32.type, s.m23.type);
        vd.visit(s.m23.type, s.m32.type);
        vd.visit(s.m23.type, s.m3.type);
        vd.visit(s.m3.type, s.m32.type);
        vd.visit(s.m23.type, s.v3.type);
        vd.visit(s.m32.type, s.v2.type);

        vd.visit(s.v2.type, s.bTrue.type);
        vd.visit(s.v2.type, s.fPi.type);
        vd.visit(s.v2.type, s.iNeg.type);
        vd.visit(s.v2.type, s.m23.type);
        vd.visit(s.v3.type, s.m32.type);
